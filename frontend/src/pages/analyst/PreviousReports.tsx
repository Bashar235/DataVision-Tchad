...
import { getAdminAudit, getReportHistory, downloadReport } from "@/services/api";
...
const PreviousReports = () => {
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      // data from getReportHistory is already pre-mapped
      const reportLogs = data.map((log: any) => ({
        id: log.id,
        title: log.report_type || log.action || "Report",
        date: new Date(log.timestamp || log.created_at).toLocaleString(),
        path: log.path,
    ...
    <div className="max-w-7xl mx-auto space-y-6 mt-4">
      <div className={`flex flex-col md:flex-row md:items-center justify-between gap-4 ${isRtl ? 'md:flex-row-reverse' : ''}`}>
        <div className="text-start">
          <h1 className="text-2xl font-bold text-foreground">{t('side_nav_previous_reports')}</h1>
