    ...
    return (
        <header className="shrink-0 bg-[#1e1f20] border-b border-[#e3e3e3]/10 shadow-sm z-40">
            <div className="flex items-center justify-between px-4 h-16">
                ...
                <div className={`flex items-center gap-3 ${isRtl ? 'flex-row-reverse' : ''}`}>
                    <LanguageSwitcher />
                    <div className={`hidden md:block ${isRtl ? 'text-left me-2 border-s border-white/10 ps-4' : 'text-right me-2 border-l border-white/10 pl-4'}`}>
                        <p className="text-sm font-medium text-white">{user?.full_name || t('dashboard_researcher')}</p>
